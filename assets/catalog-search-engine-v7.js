(function () {
  ...
  function createSearchEngine(options) {
    ...
    function productDoc(product) {
      ...
      var uniformFamily = !uniformExclude.test(conceptTitle) && /制服|学生服|標準服|せーらー|ぶれざー|学らん|詰襟|詰衿|通園服|園内服|おーばーぶらうす|ぶらうす|にっとしゃつ|かったーしゃつ|ぷりーつ.*すかーと|夏すかーと|(?:^|旧たいぷ)(?:紺色|黒色)?すかーと|すらっくす|りぼん|棒たい|ねくたい|衿かばー/.test(conceptTitle);
      var uniformRole = "";
      if (uniformFamily) {
        if (/料金|ねーむ.*入れ|刺繍.*入れ/.test(conceptTitle)) uniformRole = "service";
        else if (/せーらー(?!服?用)|学らん|詰襟|詰衿|ぶれざー|通園服|園内服/.test(conceptTitle)) uniformRole = "core";
        else if (/すかーと|すらっくす/.test(conceptTitle)) uniformRole = "bottom";
        else if (/おーばーぶらうす|ぶらうす|にっとしゃつ|かったーしゃつ/.test(conceptTitle)) uniformRole = "shirt";
        else if (/りぼん|棒たい|ねくたい|衿かばー/.test(conceptTitle)) uniformRole = "accessory";
      ...
      if (uniformFamily && /せーらー|すかーと|ぶらうす|りぼん|棒たい/.test(conceptTitle)) matchedConcepts.girls = true;
      if (uniformFamily && /男子|学らん|詰襟|詰衿/.test(conceptTitle)) matchedConcepts.boys = true;
    ...
    function conceptRankBoost(doc, concept, parsed) {
      if (concept.id === "uniform") {
        var explicitUniformLeaf = parsed.concepts.some(function (candidate) {
          return /^(?:sailor_uniform|gakuran|uniform_blazer|preschool_uniform|uniform_blouse|uniform_shirt|uniform_ribbon|uniform_necktie|uniform_collar_cover)$/.test(candidate.id);
        });
